...
開啟 `auto_fill` 可自動重跑推理，直到所有預期圖層都生成完畢：

1. **第 1 輪** 使用原始 seed — 作為主要結果
2. 節點檢查所有預期圖層是否存在（v3+head: 24 層, v3 body only: 13 層, v2: 19 層）
3. 若有缺失圖層（alpha 覆蓋率低於 `min_alpha_coverage`），**第 2 輪** 以 `seed + 1` 開始
4. 後續輪次中正確生成的缺失圖層會自動補入主要結果
5. 最多重跑 **5 輪**，或所有圖層補齊即停止
