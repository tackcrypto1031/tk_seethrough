...
| `num_runs` | 1 | 推理次數（1-5）。大於 1 時，第一輪缺失的圖層會自動從後續輪次（不同 seed）中補齊 |
| `min_alpha_coverage` | 0.01 | 圖層有效的最小 alpha 覆蓋率。低於此閾值的圖層視為缺失 |
...
設定 `num_runs > 1` 可自動執行多次推理並補齊缺失圖層：

1. **第 1 輪** 使用原始 seed — 作為主要結果
2. 第 1 輪結束後，alpha 覆蓋率低於 `min_alpha_coverage` 的圖層標記為缺失
3. **第 2 輪** 使用 `seed + 1`，**第 3 輪** 使用 `seed + 2`，依此類推
4. 後續輪次中正確生成的缺失圖層會自動補入主要結果
5. 若所有圖層都已補齊，會提前結束不再繼續
